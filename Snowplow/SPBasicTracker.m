...
+ (instancetype) build:(void(^)(id<SPBasicTrackerBuilder>builder))buildBlock {
    SPBasicTracker* tracker = [[SPBasicTracker alloc] initWithDefaultValues];
    if (buildBlock) {
        buildBlock(tracker);
    }
    [tracker setup];
...
- (void) setup {
    ...
    _emitter = [SPEmitter build:^(id<SPEmitterBuilder> builder) {
        __typeof__(self) strongSelf = weakSelf;
        if (strongSelf == nil) return;

        [builder setUrlEndpoint:strongSelf->_endpoint];
    }];

    _subject = [[SPSubject alloc] initWithPlatformContext:YES andGeoContext:YES];
    _tracker = [SPTracker build:^(id<SPTrackerBuilder> builder) {
        __typeof__(self) strongSelf = weakSelf;
        if (strongSelf == nil) return;

        [builder setEmitter:strongSelf->_emitter]; // Required
        [builder setSubject:strongSelf->_subject];
        [builder setAppId:strongSelf->_appId];
        [builder setTrackerNamespace:strongSelf->_namespace];
    }];
